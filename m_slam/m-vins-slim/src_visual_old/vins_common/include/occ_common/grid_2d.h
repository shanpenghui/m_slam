#ifndef OCCUPANCY_GRID_GRID_2D_H_
#define OCCUPANCY_GRID_GRID_2D_H_
...
class Grid2D {
 public:
    ...
    // immediately after 'FinishUpdate', before any calls to 'ApplyLookupTable'.
    virtual void GrowLimits(const Eigen::Vector2d& point);

    virtual std::unique_ptr<Grid2D> ComputeCroppedGrid(
            cv::Mat* submap_odds_ptr) const = 0;

    virtual float GetValue(const Eigen::Array2i& cell_index) const = 0;
