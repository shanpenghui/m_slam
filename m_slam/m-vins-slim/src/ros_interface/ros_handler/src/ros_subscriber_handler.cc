...
void InitializeSubscribers(
...
#else
    // Set image subscriber.
    topic_subscriber.img_sub_ = std::make_shared<ros::Subscriber>(node_handler.subscribe<ImageMsg>(
        ros_topics.at(CameraTopicName), 10,
        &Interface::FillImageMsg,
        &interface));

    // Set depth subscriber.
    topic_subscriber.depth_sub_ = std::make_shared<ros::Subscriber>(node_handler.subscribe<ImageMsg>(
        ros_topics.at(CameraDepthTopicName), 10,
        &Interface::FillDepthMsg,
        &interface));
    ...
    // Set imu subscriber.
    topic_subscriber.imu_sub_ = std::make_shared<ros::Subscriber>(node_handler.subscribe(
        ros_topics.at(ImuTopicName), 100,
        &Interface::FillImuMsg,
        &interface));

    if (interface_ptr->UseScanPointCloud()) {
        // Set scan pointcloud2 subscriber.
        topic_subscriber.scan_pc2_sub_ = std::make_shared<ros::Subscriber>(node_handler.subscribe(
            ros_topics.at(ScanCloudTopicName), 10,
                        &Interface::FillScanPc2Msg,
                        &interface));
    } else {
        // Set scan subscriber.
...
#else
    topic_subscriber.img_sub_->shutdown();
    topic_subscriber.depth_sub_->shutdown();
    topic_subscriber.imu_sub_->shutdown();
    if (topic_subscriber.scan_sub_ != nullptr) {
        topic_subscriber.scan_sub_->shutdown();
    } else {
        CHECK_NOTNULL(topic_subscriber.scan_pc2_sub_);
        topic_subscriber.scan_pc2_sub_->shutdown();
    }
