#ifndef LOOP_CLOSURE_KD_TREE_INDEX_H_
#define LOOP_CLOSURE_KD_TREE_INDEX_H_
...
class KDTreeIndex {
 public:
  ...
  KDTreeIndex(const float knn_max_radius)
      : knn_max_radius_(knn_max_radius) {
  }
  ...
  inline int GetNumDescriptorsInIndex() const {
    ...
    for (const std::shared_ptr<common::DescriptorsMatF32>& pending_block :
  ...
  void AddDescriptors(const DescriptorMatrixType& descriptors) {
    pending_descriptor_blocks_.push_back(
        aligned_shared<common::DescriptorsMatF32>(descriptors));
  }
  ...
  void RefreshIndex() const {
    ...
    for (const std::shared_ptr<common::DescriptorsMatF32>& descriptor_block :
         pending_descriptor_blocks_) {
      CHECK_NOTNULL(descriptor_block);
    ...
    VLOG(10) << "Index data size increase to: "
             << index_data_.cols();
  ...
  inline void GetNNearestNeighbors(
    ...
    index_ref.knn(
        ...
        kSearchOptionsDefault, knn_max_radius_);
  }

 protected:
  const float knn_max_radius_;
  mutable std::shared_ptr<NNSearch> index_;
  mutable common::DescriptorsMatF32 index_data_;
  mutable std::vector<std::shared_ptr<common::DescriptorsMatF32>>
      pending_descriptor_blocks_;
};
}  // namespace kd_tree_index
}  // namespace loop_closure

#endif  // LOOP_CLOSURE_KD_TREE_INDEX_H_
