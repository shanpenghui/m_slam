#ifndef MVINS_GYRO_TWO_FRAME_MATCHER_H_
#define MVINS_GYRO_TWO_FRAME_MATCHER_H_
...
public:
    ...
    /// @param[in]  config  The config params for SLAM.
    /// @param[in]  frame_data_kp1     The current VisualFrame that needs to contain the keypoints and
    ///                                  descriptor channels. Usually this is an output of the VisualPipeline.
    /// @param[in]  frame_data_k       The previous VisualFrame that needs to contain the keypoints and
    ///                                  descriptor channels. Usually this is an output of the VisualPipeline.
    ...
    /// @param[out] scored_matches_kp1_k_ptr  Vector of structs containing the found matches. Indices
...
private:
    ...
    typedef typename common::FrameToFrameMatchesWithScore::iterator MatchesIterator;
    ...
    double ComputeMatchingScore(const float num_matching_bits,
                                const float descriptor_size_bits) const;

    // Compute ratio test. Test is inspired by David Lowe's "ratio test"
    // for matching descriptors. Returns true if test is passed.
    bool RatioTest(const float max_distance,
                   const float distance_shortest,
                   const float distance_second_shortest) const;
    // Params.
    const common::SlamConfigPtr config_;
    // The current frame.
    const common::VisualFrameData &frame_kp1_;
    // The previous frame.
    const common::VisualFrameData &frame_k_;
    ...
    const size_t kDescriptorSizeBytes_;
    // Number of keypoints/descriptors in frame (k+1).
    const int kNumPointsKp1_;
    // Number of keypoints/descriptors in frame k.
    const int kNumPointsK_;
    const uint32_t kImageHeight_;
...
inline void GyroTwoFrameMatcher::GetKeypointIteratorsInWindow(
    const Eigen::Vector2d& predicted_keypoint_position,
    const int window_half_side_length_px,
    KeyPointIterator* it_keypoints_begin,
    KeyPointIterator* it_keypoints_end) const {
    ...
    int LUT_index_top = Clamp(0, kImageHeight_ - 1, static_cast<int>(
        predicted_keypoint_position(1) + 0.5 - window_half_side_length_px));
    int LUT_index_bottom = Clamp(0, kImageHeight_ - 1, static_cast<int>(
        predicted_keypoint_position(1) + 0.5 + window_half_side_length_px));

    *it_keypoints_begin = keypoints_kp1_sorted_by_y_.begin() +
                          corner_row_LUT_[LUT_index_top];
    *it_keypoints_end = keypoints_kp1_sorted_by_y_.begin() +
                        corner_row_LUT_[LUT_index_bottom];

    CHECK_LE(LUT_index_top, LUT_index_bottom);
    CHECK_GE(LUT_index_bottom, 0);
    CHECK_GE(LUT_index_top, 0);
    CHECK_LT(LUT_index_top, static_cast<int>(kImageHeight_));
    CHECK_LT(LUT_index_bottom, static_cast<int>(kImageHeight_));
}
...
inline double GyroTwoFrameMatcher::ComputeMatchingScore(
    const float num_matching_bits, const float descriptor_size_bits) const {
    return static_cast<double>(num_matching_bits) / descriptor_size_bits;
}

inline bool GyroTwoFrameMatcher::RatioTest(
    const float max_distance,
    const float distance_closest,
    const float distance_second_closest) const {
    // CHECK_LE(distance_closest, distance_second_closest);
    if (distance_closest > distance_second_closest) {
        LOG(ERROR) << "CHECK_LE failed";
        CHECK_NEAR(distance_closest, distance_second_closest, 0.001);
    }
    if (distance_second_closest == max_distance) {
        // There has never been a second matching candidate.
        // Consequently, we cannot conclude with this test.
        return true;
    } else if (distance_second_closest == 0.f) {
...
#endif // MVINS_GYRO_TWO_FRAME_MATCHER_H_
