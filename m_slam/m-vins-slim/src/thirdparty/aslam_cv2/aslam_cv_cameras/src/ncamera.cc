...
const Transformation& NCamera::get_T_BtoC(size_t camera_index) const {
  CHECK_LT(camera_index, cameras_.size());
  return T_C_B_[camera_index];
}

Transformation& NCamera::get_T_BtoC_Mutable(size_t camera_index) {
  CHECK_LT(camera_index, cameras_.size());
  return T_C_B_[camera_index];
}

const Transformation& NCamera::get_T_BtoC(const CameraId& camera_id) const {
  ...
  return get_T_BtoC(camera_idx);
}

Transformation& NCamera::get_T_BtoC_Mutable(const CameraId& camera_id) {
  CHECK(camera_id.isValid());
  int camera_idx = getCameraIndex(camera_id);
  CHECK_GE(camera_idx, 0) << "Camera with ID " << camera_id
                          << " not in NCamera! container";
  return get_T_BtoC_Mutable(camera_idx);
}

void NCamera::set_T_BtoC(size_t camera_index, const Transformation& T_Ci_B) {
