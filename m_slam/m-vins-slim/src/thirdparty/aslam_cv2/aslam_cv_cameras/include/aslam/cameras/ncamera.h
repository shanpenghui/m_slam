...
public:
  ...
  const Transformation& get_T_BtoC(size_t camera_index) const;

  /// Get the pose of body frame with respect to the camera i.
  Transformation& get_T_BtoC_Mutable(size_t camera_index);
  ...
  const Transformation& get_T_BtoC(const CameraId& camera_id) const;

  /// Get the pose of body frame with respect to the camera with a camera id.
  /// The method will assert that the camera is not in the rig!
  Transformation& get_T_BtoC_Mutable(const CameraId& camera_id);

  /// Set the pose of body frame with respect to the camera i.
  void set_T_BtoC(size_t camera_index, const Transformation& T_Ci_B);
