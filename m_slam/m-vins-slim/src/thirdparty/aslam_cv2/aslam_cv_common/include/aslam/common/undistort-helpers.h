  ...
  for (y = k = 0; y < N; y++) {
    for (x = 0; x < N; x++) {
      ...
      switch(input_camera.getType()) {
        ...
        case Camera::Type::kEquirectangular: {
          const aslam::EquirectCamera *const cam =
              dynamic_cast<const aslam::EquirectCamera*>(&input_camera);
          CHECK_NOTNULL(cam);
          camera_matrix = cam->getCameraMatrix();  // This will LOG(FATAL).
          break;
        }
        default: {
