...
std::string ConcatenateFilePathFrom(
    const std::string& directory, const std::string& file_name) {
    std::string path;
    ConcatenateFilePathFrom(directory, file_name, &path);
    return path;
}
...
int getAllFilesAndFoldersInFolder(
    ...
    if ((directory = opendir(folder.c_str())) != nullptr) {
        ...
        LOG(ERROR) << "Unable to open directory. Error: "
                   << strerror(errno) << ". "
                   << folder;
        return errno;
    }
...
int getFileLists(
    ...
    if (sort_lexical) {
        ...
        std::sort(file_paths->begin(), file_paths->end(),
              std::bind(&common::compareNumericPartsOfStrings, std::placeholders::_2,
...
bool createPath(const std::string& path_to_create_input) {
    ...
    while ((current_position = path_to_create.find_first_of(
        ...
        if ((make_dir_status = mkdir(current_directory.c_str(), kMode)) &&
                errno != EEXIST) {
            VLOG(3) << "Unable to make path! Error: " << strerror(errno);
