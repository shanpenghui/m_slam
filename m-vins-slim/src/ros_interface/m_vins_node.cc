...
if (!common::pathExists(config_file_path)) {
    LOG(ERROR) << "ERROR: slam_yaml_config must be a config directory path in the slimmed project.";
    return 1;
}

const bool online = true;
const std::string complete_config_file_name_default =
    common::ConcatenateFilePathFrom(config_file_path, kConfigIdleFileName);
