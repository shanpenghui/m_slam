...
bool convert<std::shared_ptr<aslam::Camera> >::decode(
    ...
    try {
        if (!node.IsMap()) {
            LOG(ERROR) << "Unable to get parse the camera "
                       << "because the node is not a map.";
            return true;
        }
        // Determine the distortion type.
        ...
        if (distortion_config) {
            ...
            if (YAML::safeGet(distortion_config, "type",
                    &distortion_type) && YAML::safeGet(distortion_config,
              ...
              if (distortion_type == "none") {
                  ...
                  if (aslam::EquidistantDistortion::
                          areParametersValid(distortion_parameters)) {
                      distortion.reset(new aslam::EquidistantDistortion(
                              distortion_parameters));
                  } else {
                      LOG(ERROR)
                          << "Invalid distortion parameters for"
                          << " the Equidistant distortion model: "
                          << distortion_parameters.transpose()
                          << std::endl << "See aslam::"
                          << "EquidistantDistortion::areParametersValid(...) "
                          << "for conditions on what valid Equidistant "
                          << "distortion parameters look like.";
                      return true;
                  }
              } else if (distortion_type == "fisheye") {
                if (aslam::FisheyeDistortion::areParametersValid(
                        distortion_parameters)) {
                    distortion.reset(new aslam::FisheyeDistortion(
                        distortion_parameters));
                } else {
                    LOG(ERROR)
                        << "Invalid distortion parameters "
                        << "for the Fisheye distortion model: "
                        << distortion_parameters.transpose() << std::endl
                        << "See aslam::FisheyeDistortion::"
                        << "areParametersValid(...) for conditions on what "
                  ...
                  if (aslam::RadTanDistortion::areParametersValid(
                          distortion_parameters)) {
                      distortion.reset(new aslam::RadTanDistortion(
                          distortion_parameters));
                  } else {
                      LOG(ERROR)
                          << "Invalid distortion parameters for "
                          << "the RadTan distortion model: "
                          << distortion_parameters.transpose()
                          << std::endl
                          << "See aslam::RadTanDistortion::"
                          << "areParametersValid(...) for conditions on what "
                          << "valid RadTan distortion parameters look like.";
                      return true;
                  }
              } else if (distortion_type == "plumb_bob") {
                  if (aslam::PlumbBobDistortion::areParametersValid(
                            distortion_parameters)) {
                    distortion.reset(new aslam::PlumbBobDistortion(
                            distortion_parameters));
                  } else {
                      LOG(ERROR)
                          << "Invalid distortion parameters "
                          << "for the PlumbBob distortion model: "
                          << distortion_parameters.transpose()
                          << std::endl
                          << "See aslam::PlumbBobDistortion::"
                          << "areParametersValid(...) for conditions on what "
                          << "valid PlumbBob distortion parameters look like.";
                      return true;
                  }
              } else {
                  LOG(ERROR)
                      ...
                      << "radial-tangential, plumb_bob}.";
                  return true;
              }
              if (!distortion->distortionParametersValid(
                      distortion_parameters)) {
                  LOG(ERROR)
                      << "Invalid distortion parameters: "
                      << distortion_parameters.transpose();
                ...
                LOG(ERROR)
                    << "Unable to get the required "
                    << "parameters from the distortion. "
        ...
        if (YAML::safeGet(node, "type", &camera_type) &&
            ...
            if (camera_type == "pinhole") {
                if (aslam::PinholeCamera::areParametersValid(intrinsics)) {
                    camera.reset(new aslam::PinholeCamera(
                            intrinsics,
                            image_width,
                            image_height,
                            distortion));
                    ...
                    LOG(ERROR)
                        << "Invalid intrinsics parameters for "
                        << "the Pinhole camera model: "
                        << intrinsics.transpose() << std::endl
                        << "See aslam::PinholeCamera::areParametersValid(...)"
                        << " for conditions on what "
                        << "valid Pinhole camera intrinsics look like.";
                    return true;
                }
            } else if (camera_type == "unified-projection") {
                if (aslam::UnifiedProjectionCamera::areParametersValid(
                        intrinsics)) {
                    camera.reset(new aslam::UnifiedProjectionCamera(
                            intrinsics,
                            image_width,
                            image_height,
                            distortion));
                    ...
                    LOG(ERROR)
                        << "Invalid intrinsics parameters for the "
                        << "UnifiedProjection camera model: "
                        << intrinsics.transpose() << std::endl
                        << "See aslam::UnifiedProjectionCamera::"
                        << "areParametersValid(...) for conditions on what "
                        << "valid UnifiedProjection camera "
                        << "intrinsics look like.";
                    return true;
                }
            } else if (camera_type == "equirect") {
                if (aslam::EquirectCamera::areParametersValid(intrinsics)) {
                    // Actually not used.
                    camera.reset(new aslam::EquirectCamera(
                    ...
                    LOG(ERROR)
                        ...
                        << intrinsics.transpose() << std::endl
                        << "See aslam::EquirectCamera::"
                        << "areParametersValid(...) for conditions on what "
                        << "valid Equirect camera "
                        << "intrinsics look like.";
                }
            } else {
                LOG(ERROR)
                    << "Unknown camera model: \"" << camera_type << "\". "
                    << "Valid values are {pinhole, unified-projection}.";
                return true;
            }
        } else {
            LOG(ERROR)
                << "Unable to get the required parameters from the camera. "
                << "Required: string type, int image_height, "
                << "int image_width, VectorXd intrinsics.";
            return true;
        }
        ...
        if (node["id"]) {
            ...
            if (!id.fromHexString(id_string)) {
                LOG(ERROR) << "Unable to parse \""
                           << id_string << "\" as a hex string.";
                camera.reset();
                return true;
            }
        } else {
            LOG(WARNING) << "Unable to get the id for the camera. "
                         << "Generating new random id.";
            id.randomize();
        }
        camera->setId(id);
        if (node["line-delay-nanoseconds"]) {
            uint64_t line_delay_nanoseconds =
                    camera->getLineDelayNanoSeconds();
            if (YAML::safeGet(node, "line-delay-nanoseconds",
                    &line_delay_nanoseconds)) {
                camera->setLineDelayNanoSeconds(line_delay_nanoseconds);
            } else {
                LOG(ERROR) << "Unable to parse the parameter "
        ...
        if (node["t_offset_imu_cam"]) {
            camera->setTimeOffsetImuCam(node["t_offset_imu_cam"].as<double>());
        }
    } catch(const std::exception& e) {
        LOG(ERROR) << "Yaml exception during parsing: "
                   << e.what();
        camera.reset();
        return true;
    }
...
Node convert<aslam::Camera::Ptr>::encode(
        const aslam::Camera::Ptr& camera) {
  return convert<aslam::Camera>::encode(
          *CHECK_NOTNULL(camera.get()));
}

bool convert<aslam::Camera>::decode(
        const Node&,  /*node*/
        aslam::Camera&  /*camera*/) {
...
Node convert<aslam::Camera>::encode(
    ...
    camera_node["t_offset_imu_cam"] = camera.getTimeOffsetImuCam();
    ...
    switch (camera.getType()) {
        ...
        case aslam::Camera::Type::kUnifiedProjection:
            ...
            break;
        case aslam::Camera::Type::kEquirectangular:
            camera_node["type"] = "equirect";
        default:
            LOG(ERROR)
                << "Unknown camera model: "
                << static_cast<std::underlying_type<
                        aslam::Camera::Type>::type>(camera.getType());
    }
    camera_node["intrinsics"] = camera.getParameters();

    const aslam::Distortion& distortion =
            camera.getDistortion();
    if (distortion.getType() !=
      ...
      switch (distortion.getType()) {
          ...
          case aslam::Distortion::Type::kRadTan:
              ...
              break;
          case aslam::Distortion::Type::kPlumbBob:
              distortion_node["type"] = "plumb-bob";
              break;
          default:
              LOG(ERROR) << "Unknown distortion model: "
                  << static_cast<std::underlying_type<
                          aslam::Distortion::Type>::type>(
                          distortion.getType());
      }
      distortion_node["parameters"] =
